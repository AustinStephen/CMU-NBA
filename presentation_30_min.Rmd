...
```{r Flight Paths, echo = FALSE, fig.height = 7, fig.width=10}
...
2. Offer framework for analyzing fatigue from a
...
check$type_of_rest <- case_when(
  x >= 10 ~ "extended injury rest (10+)",
  x < 10 & x >= 4 ~ "minor injury rest (4-9)", 
  x < 4 & x > 1 ~ "2-3 extra days rest",
  x == 1 ~ "1 game load manage",
  x == 0 ~ "no load manage",
)

check$type_of_rest <- ordered(check$type_of_rest, levels = c("extended injury rest (10+)",
                                                             "minor injury rest (4-9)",
                                                             "2-3 extra days rest",
                                                             "1 game load manage",
                                                             "no load manage"))
#games <- c(1:576)
...
ggplot(data = filter(check, ratioPIE < 0.6)) +
  geom_point(aes(x = game, y = ratioPIE, color = type_of_rest, shape = w_l), alpha = 0.7) +
...
### 3 games in 4 days difference of means
...
- Remove variance in response associated with team strength
...
### Rest, 3 in 4, and 3 hours back were significant
 <font size="5"> $gameNetRatingResidual$ = $\beta_0$ + $\beta_1$ $restDiff$ + $\beta_2$ $travel3HoursBack$ + $\beta_3$ $third GameFourDays$ </font>
...
betas <- as.numeric(summ$coefficients[,1])[c(1,3,4,5)]
...
<br>
* Generalize load management as a strategy for more players
<br>
...
**Special thanks** to Tom Bliss as well for his insight on fatigue analysis in the NFL
